...
export function LoginRegister() {
  ...
  return (
    <div className="login-register">
      {token? 
      <button className = "login-out button-right" onClick={() => setSureLogOut(true)}>Cerrar sesión</button>
      :
      <>
      <Link to={"/login"}>
      ...
      <div className={`logout-confirmation ${sureLogOut ? 'visible' : ''}`} onClick={() => setSureLogOut(false)}>
        <div className="logout-content" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="logout-buttons">
            <button className="confirm-logout-button button-right" onClick={handleLogout}>Sí</button>
            <button className="cancel-logout-button button-right" onClick={() => setSureLogOut(false)}>No</button>
          </div>
